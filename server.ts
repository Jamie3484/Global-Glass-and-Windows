...
function getDomainExpertResponse(userMessage: string): string {
  ...
  let body = '';
  if (text.includes('prix') || text.includes('tarif') || text.includes('cout') || text.includes('coût') || text.includes('combien') || text.includes('devis')) {
    body = `### 📋 Devis sur Mesure — GLOBAL GLASS AND WINDOWS\n\nChez **GLOBAL GLASS AND WINDOWS**, l'ensemble de nos fabrications (portes, fenêtres, miroirs, vitrines et cabines de douche) sont réalisées **sur mesure** selon vos dimensions et spécifications techniques précises.\n\n**Comment obtenir votre devis personnalisé gratuit :**\n1. Remplissez le formulaire de **Demande de Devis** en ligne en précisant vos dimensions (longueur × largeur en pouces).\n2. Ou contactez notre atelier directement :\n   • +(509) 4467-5506\n   • +(509) 3599-5664\n   • +(509) 3687-0000\n3. Nos experts vous prépareront un chiffrage détaillé adapté à votre projet.`;
    ...
    body = `### 📐 Formule Officielle de Calcul de Surface Vitrée\n\nChez **GLOBAL GLASS AND WINDOWS**, le calcul des surfaces nécessaires se base sur les dimensions en pouces :\n\n$$\\text{Surface en pieds carrés (pi²)} = \\frac{\\text{Longueur (pouces)} \\times \\text{Largeur (pouces)}}{144}$$\n\n**Exemple pratique :**\nPour une ouverture de 48 pouces de haut par 36 pouces de large :\n$$\\frac{48 \\times 36}{144} = \\frac{1728}{144} = \\mathbf{12 \\text{ pi²}}$$\nCette surface permet à nos vitriers de couper vos verres avec une précision millimétrique.`;
